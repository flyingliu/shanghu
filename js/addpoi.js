...
function joinline() {
  ...
  var postdata = "line_" + linename + "    " + brower + "    " + lineArr;
  $.cookie("line_" + linename, lineArr);
  $.post("http://tmp.php.demo.org.cn/maptag.php",{line:postdata},function(status){
    if(status){
      showline();
      $(".linename").val('');
      lineArr = [];
...
function onComplete(data) {
  ...
  $("#jingdu").html(parseFloat(data.accuracy).toFixed(2));
